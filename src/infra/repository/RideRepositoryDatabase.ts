...
export default class RideRepositoryDatabase implements RideRepository {
    ...
    async hasActiveRideByPassengerId(passengerId: string): Promise<boolean> {
        const [rideData] = await this.connection.query("select count(*)::int as count from ride where passenger_id = $1 and status in ('requested', 'accepted', 'in_progress')", [passengerId])
        return rideData.count > 0;
    }

    async hasActiveRideByDriverId(driverId: string): Promise<boolean> {
        const [rideData] = await this.connection.query("select count(*)::int as count from ride where driver_id = $1 and status in ('accepted', 'in_progress')", [driverId])
        return rideData.count > 0;
    }

    async updateRide(ride: Ride): Promise<void> {
        await this.connection.query("update ride set driver_id = $1, status = $2, distance = $3, fare = $4 where ride_id = $5", [ride.driverId, ride.status, ride.distance, ride.fare, ride.rideId,]);
    }
}
